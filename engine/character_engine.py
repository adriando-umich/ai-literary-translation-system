...
class CharacterEngine:
    ...
    def _call_llm(self, *, system_prompt: str, user_prompt: str) -> str:
        # Cấu hình tắt bộ lọc (BLOCK_NONE) - GIỮ NGUYÊN
        safety_settings = [
        ...
        # --- PHẦN SỬA ĐỔI CHO FALLBACK ---
        for attempt in range(1, CHARACTER_MAX_RETRIES + 1):
            # Quyết định model dựa trên số lần thử (Attempt)
            # Với MAX_RETRIES = 4: Attempt 1,2 dùng 2.0-flash | Attempt 3,4 dùng 3-flash-preview
            current_model = self.model
            if attempt > (CHARACTER_MAX_RETRIES // 2):
                current_model = "gemini-3-flash-preview"

            try:
                log(f"CHARACTER_ENGINE: API call attempt {attempt} using [{current_model}]")

                response = self.client.models.generate_content(
                    model=current_model,  # Sử dụng biến current_model thay vì self.model
                ...
                last_error = e
                log(f"⚠️ CHARACTER_ENGINE API ERROR attempt {attempt} with [{current_model}]: {e}")
        ...
        raise RuntimeError(
            f"CHARACTER_ENGINE FAILED after {CHARACTER_MAX_RETRIES} attempts (Primary & Fallback failed)") from last_error
